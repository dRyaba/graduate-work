/**
 * @file CliHandlers.cpp
 * @brief Implementation of CLI subcommand handlers.
 *
 * Extracted verbatim from main.cpp to keep the entry point focused on
 * dispatch and startup boilerplate.
 */
...
#endif
    std::cout << "\nParameters for --cross-check:\n";
    std::cout << "  --timeout <N>  Per-cell timeout in seconds (default 30 = tiered per method)\n";
    std::cout << "  --output <F>   Output CSV path (default cross_check_results.csv)\n";
    std::cout << "  --quick        Skip large real-world networks (Geant2004, IEEE-118)\n";
...
int handleCrossCheck(const std::vector<std::string>& args) {
    int timeout_sec = 30;
    std::string output_file = "cross_check_results.csv";
    ...
    auto parse_positive_int = [](const std::string& raw, const char* flag,
        ...
        try {
            ...
            std::cerr << "Error: invalid " << flag << " value\n";
            return false;
        }
        if (out < 1) {
            std::cerr << "Error: " << flag << " must be >= 1\n";
            return false;
        }
        return true;
    };

    auto parse_methods = [](const std::string& raw,
        ...
        for (size_t i = 0; i <= raw.size(); ++i) {
            ...
            if (c == ',') {
                ...
                int m;
                try { m = std::stoi(token); }
                catch (...) {
                    std::cerr << "Error: invalid --methods token: " << token << "\n";
                    return false;
                }
                if (m < 0 || m > 5) {
                    std::cerr << "Error: --methods values must be in 0..5 (got "
                              << m << ")\n";
                    return false;
                }
                out.push_back(m);
        ...
        if (out.empty()) {
            std::cerr << "Error: --methods needs at least one method id\n";
            return false;
        }
    ...
    for (size_t i = 1; i < args.size(); ++i) {
        if (args[i] == "--timeout" && i + 1 < args.size()) {
            ...
            include_real_networks = false;
        } else if (args[i] == "--d-count" && i + 1 < args.size()) {
            if (!parse_positive_int(args[++i], "--d-count", d_count))
                return 1;
        } else if (args[i] == "--d-step" && i + 1 < args.size()) {
            if (!parse_positive_int(args[++i], "--d-step", d_step))
                return 1;
            ...
            std::cerr << "Error: unknown cross-check argument: " << args[i] << "\n";
            return 1;
        }
    }
    ...
    try {
        ...
        std::cout << "All methods agree within tolerance.\n";
    } catch (const std::exception& e) {
        LOG_ERROR("Exception during cross-check: {}", e.what());
        std::cerr << "Error during cross-check: " << e.what() << "\n";
        return 1;
    }
