...
// =============================================================================
// CancelaPetingiStateMulti Implementation
// =============================================================================

CancelaPetingiStateMulti::CancelaPetingiStateMulti(const std::vector<Path>& paths, 
                                                   const Graph& graph, 
                                                   int max_diameter)
    : paths_(paths), max_diameter_(max_diameter) {
    
    size_t num_paths = paths.size();
    linksp_.resize(num_paths);
    feasible_.resize(num_paths, true);
    path_lengths_.resize(num_paths);
    
    npst_by_length_.resize(max_diameter + 1, 0);
    connected_by_length_.resize(max_diameter + 1, false);
    min_perfect_path_length_ = max_diameter + 1;

    std::set<EdgeId> edge_set;
    for (size_t pi = 0; pi < num_paths; ++pi) {
        path_lengths_[pi] = static_cast<int>(paths[pi].size());
        for (EdgeId e : paths[pi]) {
            edge_set.insert(e);
        }
    }
    ...
    buildPOfEdge(paths);
    updateConnectedByLength();
}

void CancelaPetingiStateMulti::buildPOfEdge(const std::vector<Path>& paths) {
    P_of_edge_.clear();
    for (size_t pi = 0; pi < paths.size(); ++pi) {
        for (EdgeId e : paths[pi]) {
            P_of_edge_[e].push_back(pi);
...
void CancelaPetingiStateMulti::updateConnectedByLength() {
    std::fill(connected_by_length_.begin(), connected_by_length_.end(), false);
    min_perfect_path_length_ = max_diameter_ + 1;
    
    for (size_t pi = 0; pi < paths_.size(); ++pi) {
        if (!feasible_[pi]) continue;
        if (linksp_[pi] == 0) {
            int len = path_lengths_[pi];
            if (len <= max_diameter_) {
                connected_by_length_[len] = true;
                if (len < min_perfect_path_length_) {
                    min_perfect_path_length_ = len;
                }
            }
        }
    }
    
    for (int d = 1; d <= max_diameter_; ++d) {
        if (connected_by_length_[d - 1]) {
            connected_by_length_[d] = true;
        }
    }
}

void CancelaPetingiStateMulti::applyContract(EdgeId e) {
    auto it = P_of_edge_.find(e);
    if (it == P_of_edge_.end()) return;
    ...
    for (size_t pi : it->second) {
        ...
        if (linksp_[pi] == 0) {
            ...
            if (len <= max_diameter_) {
                ...
                for (int d = len; d <= max_diameter_; ++d) {
                    connected_by_length_[d] = true;
                }
            }
        }
...
void CancelaPetingiStateMulti::applyDelete(EdgeId e) {
    auto it = P_of_edge_.find(e);
    if (it == P_of_edge_.end()) return;
...
bool CancelaPetingiStateMulti::isTerminalSuccess(int d) const {
    if (d < 0 || d > max_diameter_) return false;
    return connected_by_length_[d];
}

bool CancelaPetingiStateMulti::isTerminalFail(int d) const {
    for (int len = 0; len <= d && len <= max_diameter_; ++len) {
        if (npst_by_length_[len] > 0) return false;
    }
    return true;
}

bool CancelaPetingiStateMulti::allPathsInfeasible() const {
    for (int len = 0; len <= max_diameter_; ++len) {
        if (npst_by_length_[len] > 0) return false;
    }
    return true;
}

int CancelaPetingiStateMulti::minFeasiblePathLength() const {
    for (int len = 0; len <= max_diameter_; ++len) {
        if (npst_by_length_[len] > 0) return len;
    }
    return max_diameter_ + 1;
}

size_t CancelaPetingiStateMulti::countFeasiblePathsInP(EdgeId e) const {
    auto it = P_of_edge_.find(e);
    if (it == P_of_edge_.end()) return 0;
    size_t count = 0;
    for (size_t pi : it->second) {
...
CancelaPetingiStateMulti::EdgeId CancelaPetingiStateMulti::selectPivotEdge() const {
    EdgeId best = static_cast<EdgeId>(-1);
    size_t best_count = 0;
    
    for (const auto& [e, r] : edge_reliability_) {
        if (r > 0 && r < 1.0) {
            size_t count = countFeasiblePathsInP(e);
            if (count > best_count || (count == best_count && (best == static_cast<EdgeId>(-1) || e < best))) {
                best = e;
                best_count = count;
            }
        }
...
int CancelaPetingiStateMulti::applyISPT(EdgeId e) {
    auto it_e = P_of_edge_.find(e);
    if (it_e == P_of_edge_.end()) return 0;
...
int CancelaPetingiStateMulti::applyGlobalISPT() {
    int total_merged = 0;
